...
chemin_fichier = 'camera_euler_nicococo_5.csv';
...
colonne_1 = T(:, 1).Variables*0.01 - 3.5 -2.06;  % Première colonne
colonne_2 = [];
colonne_3 = [];
colonne_4 = [];
for i=1:numel(colonne_1)
    ...
    if rad2deg(eul(1,1))<0
        colonne_2(i) = rad2deg(eul(1,1)) +180 -2;
    else
        colonne_2(i) = rad2deg(eul(1,1)) - 180 -2;
    end
    % if rad2deg(eul(1,3))<0
    %     colonne_4(i) = rad2deg(eul(1,3)) +180;
    % else
    %     colonne_4(i) = rad2deg(eul(1,3)) - 180;
    % end
    % colonne_2(i) = rad2deg(eul(1,1));
    colonne_3(i) = rad2deg(eul(1,2))+3;
    colonne_4(i) = rad2deg(eul(1,3))+1.5;
...
for i = 1:numel(noms_des_cles)
    ...
    camera_meas(i,3) = b(2,1);
    camera_meas(i,4) = b(3);
    if b(3)<0
        camera_meas(i,4) = b(3) +180;
    else
        camera_meas(i,4) = b(3) - 180;
    end
    % camera_meas(i,5) = b(4);
